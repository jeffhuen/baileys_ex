...
- **Connection Phase 6 is in progress**: config defaults, the 3-byte frame codec, a real Mint-backed WebSocket transport, the socket's Noise handshake path to `:connected`, the post-handshake `connection.update`/keep-alive/logout/offline/presence foundations, a reconnecting wrapper, an ETS-backed runtime store, and the first `AwaitingInitialSync` buffer timeout choreography now exist; the remaining rc.9 parity work is QR/pair-success auth flow, dirty/init-query handling, and the rest of the `chats.ts` sync-state runtime
- Targets Elixir 1.19+ / OTP 28

> **Status:** Phases 1-3 are complete for their current scope, Phase 4 has a reference-aligned Noise implementation, Phase 5 is complete for its adapter-driven scope, and Phase 6 is in progress with the connection config, frame codec, Mint transport boundary, post-handshake socket runtime foundations, reconnect/store/event runtime wiring, and the first sync-buffer foundation in-tree. The remaining Phase 6 work is the rest of the Baileys 7.0.0-rc.9 connection/runtime contract above that foundation, while broad WAProto message/auth code generation remains deferred to the later phases that consume it. See `dev/implementation_plan/PROGRESS.md`.
