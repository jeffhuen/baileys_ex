...
- [ ] Add an explicit Phase 7 follow-up note so this phase reads as architecture alignment, not as a silent reopening of delivered auth behaviour.
- [ ] Keep the "Baileys is the spec" rule tied to observable behaviour, public compatibility promises, and named helper contracts.
- [ ] Update overview/dependency-map text so Phase 14 and Phase 15 are both reflected accurately.
...
- [ ] Store credentials and key-store datasets as ETF binaries on disk.
- [ ] Use crash-safe write semantics (temp file + flush/fsync + atomic rename) rather than best-effort overwrite.
- [ ] Provide the same `use_*_auth_state` helper shape as the compatibility backend so callers can opt in without custom glue.
- [ ] Keep the backend file-based and restart-safe; do not substitute an ETS-only or runtime-only approach.
...
- [ ] Remove generic atom/module reconstruction as the persistence mechanism.
- [ ] Encode/decode each persisted family (`creds`, `session`, `sender-key`, `device-list`, etc.) through explicit known JSON shapes.
- [ ] Keep compatibility with Baileys-style buffer tagging and per-file naming.
- [ ] Preserve the observable helper contract of `use_multi_file_auth_state/1`.
...
- [ ] Version the persisted format for each built-in backend.
- [ ] Load the current shipped JSON layout without forcing users to delete auth state.
- [ ] Provide a one-step migration path from compatibility JSON to native durable storage.
- [ ] Fail clearly on corrupt or unsupported data rather than silently dropping auth state.
...
- [ ] Keep the Baileys-compatible helper available and documented as such.
- [ ] Recommend the native durable backend for Elixir-first applications.
- [ ] Explain that custom SQL/NoSQL backends remain supported via behaviour without making them mandatory for this phase.
...
- [ ] Add shared contract tests that exercise both backends through the same logical datasets.
- [ ] Keep fresh-VM tests for the compatibility loader so warm-VM state never masks failures.
- [ ] Add crash-survivability tests for native file writes, including interrupted-write and recovery paths where practical.
- [ ] Verify the connection/runtime behaviour is backend-neutral for built-in helper usage.
