...
- [ ] Add a Phase 5 follow-up note so this phase reads as a contract cleanup, not a protocol reopening.
- [ ] Update the overview/dependency map to include Phase 16 as a post-Phase-15 architecture follow-up.
- [ ] Record clearly that this is behavior-neutral for standard consumers using the built-in store/auth path, but a breaking contract update for custom `signal_store_module` implementers.
...
- [ ] Replace the zero-argument `transaction/3` callback contract with an explicit transaction-store argument.
- [ ] Update `Store.transaction/3` facade types/docs so the new contract is unambiguous.
- [ ] Keep `get/3`, `set/2`, `clear/1`, and `in_transaction?/1` behaviorally consistent from the caller's point of view.
- [ ] Add focused contract tests for explicit transaction-handle semantics and nested transaction reuse.
...
- [ ] Remove `Process.get/put/delete` transaction state from the memory store.
- [ ] Store transaction cache/mutation state on the explicit transaction handle or its owned runtime data structure.
- [ ] Preserve per-key serialization, nested transaction reuse, and current read/write semantics.
- [ ] Extend tests to prove the new transaction handle carries the state previously hidden in the caller process.
...
- [ ] Remove `Process.get/put/delete` transaction state from `Auth.KeyStore`.
- [ ] Keep ETS-backed hot reads and cached misses.
- [ ] Preserve rollback to the previous persisted snapshot when batch commit fails.
- [ ] Preserve pre-key deletion safeguards and transaction-key locking behavior.
- [ ] Add tests that prove the explicit transaction handle preserves commit/rollback correctness.
...
- [ ] Rewrite every `Store.transaction(store, key, fn -> ... end)` callsite to the explicit transaction-store form.
- [ ] Update nested transaction callsites to reuse the explicit transaction handle instead of relying on hidden caller state.
- [ ] Update test-only store implementations and fakes to match the new contract.
- [ ] Confirm no `Signal.Store` implementation or internal consumer relies on hidden transaction state after the migration.
...
- [ ] Document that standard consumers using built-in helpers and `connect/2` do not need workflow or API changes.
- [ ] Document that custom `signal_store_module` implementers must update to the new explicit transaction-store contract.
- [ ] Keep built-in helper/runtime tests proving the connection stack remains behavior-neutral for normal consumers.
- [ ] Ensure all public-facing docs describe the same store/persistence story after the redesign.
