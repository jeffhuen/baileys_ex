...
- **Baileys v7.0.0-rc14 is the spec.** `dev/reference/Baileys-master/` is the authoritative
...
- The implementation must match Baileys v7.0.0-rc14 observable behavior for the same inputs: wire nodes, event output, helper return shape, config semantics, and error behavior.
...
4. **Check acceptance criteria** at the end of each phase — all must pass before
   moving on.
5. **Baileys is the spec** — `dev/reference/Baileys-master/` (v7.0.0-rc14) is the
